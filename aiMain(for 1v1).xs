   ...
   gMapNames = xsArrayCreateString(50, "", "Map names");
     ...
     //Giant Buddha, Golden Pavillion, Shogunate, Torii Gates, Toshogu Shrine 
      ...
      //Confucian Academy, Porcelain Tower, Summer Palace, Temple of Heaven, White Pagoda
          ...
          xsArraySetInt(gAsianWonders, 1, cUnitTypeypWCConfucianAcademy3);//cUnitTypeypWCPorcelainTower3
          xsArraySetInt(gAsianWonders, 2, cUnitTypeypWCPorcelainTower4);//ypWCTempleOfHeaven4
...
void setConsulateArmyPreference() {
  ...
  if (kbTechGetStatus(cTechypBigConsulateJapanese) == cTechStatusActive) {
    ...
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeypConsulateRonin, 0.1);
  }
...
int getAllyCount2()
{
   ...
   for (player=1; < cNumberPlayers)
   {
      ...
      if ((kbHasPlayerLost(player) == false) && (kbIsPlayerAlly(player) == true))
         retVal = retVal + 1;
   }
...
void updateEscrows(void)
{
   ...
   switch(kbGetAge())
   {
      ...
      case cAge2:
      {
		 econPercent = 0.4 - (0.2 * btRushBoom);  // 10% rushers, 80% boomers
		 break;
      }
      case cAge3:
      {
         econPercent = 0.35 + (0.15 * btRushBoom);  // 0.3,  +/- up to 0.15, + up to 0.3 if we have no vills.
   ...
   {     
      ...
      if (cvOkToTrainArmy == true)
      {
		 ...
		 /*if(kbGetCiv() == cCivChinese)
		    gMainAttackGoal = createSimpleAttackGoal("AttackGoal", -1, gLandUnitPicker, -1, cAge3, -1, gMainBase, false);
		 else
		    gMainAttackGoal = createSimpleAttackGoal("AttackGoal", -1, gLandUnitPicker, -1, cAge2, -1, gMainBase, false);*/
...
rule preMilitaryManager
inactive
minInterval 5
{
   
   if(kbGetAge() > cAge2 && cMyCiv == cCivChinese && xsIsRuleEnabled("militaryManager") == false)
   {
...
minInterval 10
{
   if (kbGetAge() >= cAge3)
   {
	  ...
	  //japan: send wagon next if not yet done
	  if(cMyCiv == cCivJapanese && gCardSent == 0)
	  {
         for (i=0; < aiHCDeckGetNumberCards(gDefaultDeck))
   	     {
   		    if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCShipBerryWagon2")
   			   xsArraySetInt(gCardValues, i, 999);
		 }
	  }
	  //sioux wood crates
	  if(cMyCiv == cCivXPSioux && gCardSent == 1)
	  {
         for (i=0; < aiHCDeckGetNumberCards(gDefaultDeck))
   	     {
   		    if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCShipWoodCrates4")
   			   xsArraySetInt(gCardValues, i, 899);
		 }
	  }
...
//      effectiveAge = cAge2;
   ...
   switch(kbGetAge())
   {
      case cAge1:
      {
		 ...
		 if(cMyCiv == cCivIndians && aiPlanGetState(gAgeUpResearchPlan) >= 0)
		 {
      ...
      case cAge2:
      {
		 ...
		 //wait wood crates...
		 if(cMyCiv == cCivXPAztec || cMyCiv == cCivIndians || cMyCiv == cCivOttomans)
		 {
		    if((xsGetTime() - gAgeUpTime) < 90000)
			   xsArraySetFloat(gForecasts, cResourceWood, xsArrayGetFloat(gForecasts, cResourceWood)/2);
		 }
		 if(cMyCiv == cCivJapanese || cMyCiv == cCivFrench)
		 {
		    if((xsGetTime() - gAgeUpTime) < 180000)
			   xsArraySetFloat(gForecasts, cResourceWood, xsArrayGetFloat(gForecasts, cResourceWood)/2);
		 }
		 if(cMyCiv == cCivXPSioux)
		    xsArraySetFloat(gForecasts, cResourceWood, xsArrayGetFloat(gForecasts, cResourceWood)/2);
      ...
      case cAge3:
      {
         ...
         if (cMyCiv != cCivOttomans && cMyCiv != cCivGermans)   // Ottomans get them free
         {
            addItemToForecasts(gEconUnit, 5);
         }
...
{ 
  ...
  if (cMyCiv == cCivIndians) { //Elephants are soooo expensive
    kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeypRajput, 0.0);
	//kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeypSepoy, 0.75);
  ...
  if (cMyCiv == cCivJapanese) {
	kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeypKensei, 0.0);
	kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeypAshigaru, 0.9);
  ...
  if (cMyCiv == cCivFrench) {
	...
	kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeMusketeer, 0.75);
	kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeCrossbowman, 0.25);
	kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeHalberdier, 0.0);
	kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypePikeman, 0.25);
	kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeDragoon, 0.5);
  ...
  if (cMyCiv == cCivXPAztec) {
	if (kbGetAge() < cAge4)
	   kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypexpPumaMan, 0.75);
...
{ 
   ...
   if (kbGetBuildLimit(cMyID, gHouseUnit) <= kbUnitCount(cMyID, gHouseUnit, cUnitStateAlive))
	...
	if(kbGetAge() == cAge1)
	   n = 10;
	//else if(kbGetCiv() == cCivIndians || kbGetCiv() == cCivXPAztec)
	    //n = 30;
	//else if(kbGetCiv() == cCivBritish || kbGetCiv() == cCivDutch)
	    //n = 35;
	else if(kbGetCiv() == cCivXPIroquois)
	    n = 20;
	else if(kbGetCiv() == cCivOttomans && kbGetAge() == cAge2)
	    n = 20;
	//else if(kbGetCiv() == cCivGermans && kbGetAge() == cAge2)
	    //n = 20;
...
minInterval 6
{
   ...
   else if(((xsGetTime() - start) > 40000) && (kbGetPopCap() <= kbGetPop()))
   {
      xsDisableSelf();
	  if (xsIsRuleEnabled("houseMonitor") == false)
         xsEnableRule("houseMonitor");
...
{ 
  ...
  if (kbUnitCount(cMyID, cUnitTypeypConsulate, cUnitStateAlive) < 1)
     return;
  if(cMyCiv == cCivIndians){
     if(kbTechGetStatus(cTechYPHCSmoothRelationsIndians) != cTechStatusActive)
	    return;
	 switch(gIndConsulateProc){
	    ...
	    case 2:
	    {
		   ...
		   else if(kbGetAge() > cAge3 || kbTechGetStatus(cTechypConsulateFrenchWoodCrates) == cTechStatusActive){
		       createSimpleResearchPlan(cTechypBigSequesterFrench, cUnitTypeypConsulate, cMilitaryEscrowID, 50);
		   }
		   break;
	    }
...
minInterval 5
{
   ...
   else if (kbGetCiv() == cCivXPAztec && check3 == 0)
   {
		check3 = 1;
		...
		createSimpleBuildPlan(cUnitTypeWarHut, 2, 71, false, cMilitaryEscrowID, kbBaseGetMainID(cMyID), 1); 
   }
   ...
   {  // Natives, at least 1 war hut
	...
	//double barrack
	planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeBarracks);
	if (cMyCiv == cCivBritish || cMyCiv == cCivDutch || cMyCiv == cCivSpanish || cMyCiv == cCivPortuguese || cMyCiv == cCivOttomans || cMyCiv == cCivFrench)
    {
	...
	planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeWarHut);
	if (cMyCiv == cCivXPAztec && kbUnitCount(cMyID, cUnitTypexpCoyoteMan, cUnitStateAlive) > 5 && check2 == 0)
    {
	  if (kbUnitCount(cMyID, cUnitTypeWarHut, cUnitStateAlive) < 2)
      {     // Start a new one
         if(planID >= 0)
		    aiPlanDestroy(planID);
		 createSimpleBuildPlan(cUnitTypeWarHut, 1, 70, true, cEconomyEscrowID, kbBaseGetMainID(cMyID), 1);
...
int fireDanceCheck()
{
   ...
   if(sf < 0)
      return (-1);
   targetLocation = kbUnitGetPosition(sf);
   int tf = getUnitCountByLocation(cUnitTypeLogicalTypeLandMilitary, cPlayerRelationEnemy, cUnitStateAlive, targetLocation, 20.0);
   return(tf);
}
...
//   if ( gDefenseReflexBaseID == kbBaseGetMainID(cMyID) )
  //    want = 0;
   ...
   else if((kbUnitCount(cMyID, cUnitTypexpMedicineManAztec, cUnitStateAlive) > 0) || (kbGetAge() == cAge2))
   {
   ...
   //add
   //az
   if ((cMyCiv == cCivXPAztec) && (kbGetAge() <= cAge2) && (kbUnitCount(cMyID, cUnitTypexpMedicineManAztec, cUnitStateAlive) < 2)) 
   {
   ...
   if ((cMyCiv == cCivXPAztec) && (kbGetAge() < cAge4)) 
   {
      aiPlanSetVariableInt(gNativeDancePlan, cNativeResearchPlanTacticID, 0, cTacticGiftDance);
      lastTactic = cTacticGiftDance;
      lastTacticTime = xsGetTime();
      ...
      case 0:  // Init arrays
      {
         ...
         {  // First, set maximum priority for the "best" cards 
            ...
            {  
			   ...
			   (tech == cTechYPHCFencingSchoolIndians) ||
      ...
      {  
         ...
         for (card=0; < maxCards)
         {
            if (((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCShipSettlersAsian2") ) ||
                ((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCShipBerryWagon2") ) || //YPHCShipSamurai2
                ((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCYumiDamage") ) || //YPHCSamuraiDamage
                ((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCYumiRange") ) || //YPHCSamuraiSpeed
				((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCNaginataHitpoints") ) ||
				((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "ypHCShipWoodCrates4") ) ||
				((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCAshigaruDamage") ) || //YPHCJapaneseSiege
				((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCShipShogunate") ) ||
				((kbGetCiv() == cCivJapanese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCAshigaruAntiCavalryDamage") ) || //YPHCNobleCombat
                ...
                ((kbGetCiv() == cCivIndians) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCSmoothRelationsIndians") ) || //YPHCElephantTrampling
				...
				((kbGetCiv() == cCivIndians) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCFencingSchoolIndians") ) || //YPHCGrazing
				...
				((kbGetCiv() == cCivIndians) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "YPHCShipFoodCrates3Indians") ) || //YPHCShipRhino1
				...
				((kbGetCiv() == cCivChinese) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCShipFoodCrates4") ) || //HCRefrigeration
                ...
                ((kbGetCiv() == cCivBritish) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCShipCoinCrates3") ) ||
				...
				((kbGetCiv() == cCivBritish) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCMusketeerGrenadierHitpointsBritishTeam") ) || //HCShipSettlers4
                ...
                //((kbGetCiv() == cCivOttomans) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCShipCoveredWagons") ) || //HCShipWoodCrates3
				...
				((kbGetCiv() == cCivXPSioux) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCXPShipVillagers4") ) || //HCXPTeamFoodCrates1
				...
				((kbGetCiv() == cCivXPAztec) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCXPShipMedicineMen2Aztec") ) ||
				((kbGetCiv() == cCivXPAztec) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCShipWoodCrates3") ) ||
				((kbGetCiv() == cCivXPAztec) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCXPCoinCratesAztec3") ) ||
                ((kbGetCiv() == cCivXPAztec) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCShipWoodCrates4") ) || //HCXPShipVillagers5
				...
				((kbGetCiv() == cCivXPAztec) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCXPWarChiefAztec2") ) ||
                ...
                ((kbGetCiv() == cCivXPAztec) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCShipFoodCrates3") ) ||
				((kbGetCiv() == cCivXPAztec) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCXPArtificialIslands") ) ||
				...
				((kbGetCiv() == cCivXPAztec) && (kbGetTechName(aiHCCardsGetCardTechID(card)) == "HCShipWoodCrates2") ) ||
...
minInterval 10
{
   ...
   if(kbGetAge() == cAge2 && cMyCiv == cCivOttomans && (xsGetTime() - gAgeUpTime) < 180000)
   {
      food2 = gAgeNeedFood+500;
	  gold2 = gAgeNeedFood+500;
   }
   if(kbGetAge() == cAge2 && cMyCiv == cCivXPSioux && (xsGetTime() - gAgeUpTime) < 120000)
   {
      food2 = gAgeNeedFood+500;
	  gold2 = gAgeNeedFood+500;
   }
  ...
  if ( civIsAsian() == false ) {
	 ...
	 if((cMyCiv == cCivFrench) && (kbGetAge() == cAge1))
	    politician =cTechPoliticianQuartermaster;
	 if((cMyCiv == cCivFrench) && (kbGetAge() == cAge2))//cTechPoliticianScout
	    politician =cTechPoliticianMarksman;
         ...
         for (i=0; < totalCards)
   	     {
   		    ...
   		    if ((kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCShipFune2") &&
			    ...
			    (mapType < 400))
	   		   totalValue = 200;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCShipBerryWagon2")
			   totalValue = 602;
			   ...
			   totalValue = 899;
		    else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCJapaneseMonkRangeAura")
			   totalValue = 898;
			   ...
			   totalValue = 904;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCShipShogunate")
	   		   totalValue = 1201;
         ...
         for (i=0; < totalCards)
   	     {
   		    ...
   		    if ((kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCEuropeanCannons") &&
			    ...
			    (mapType > 400))
	   		   ...
	   		   totalValue = 904;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCArtilleryCombatChinese")//YPHCAccupuncture
	   		   totalValue = 903;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCRainbowTrickle")
	   		   totalValue = 301;
		    else
         ...
         for (i=0; < totalCards)
   	     {
   		    ...
   		    if ((kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCShipCaravel2") &&
			    ...
			    (mapType > 400))
   			   totalValue = 202;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCFencingSchoolIndians")
	   		   totalValue = 605;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCForeignLogging")
	   		   totalValue = 606;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "YPHCSmoothRelationsIndians")
	   		   totalValue = 607;
         ...
         for (i=0; < totalCards)
   	     {
   		    ...
   		    if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCNavalCombat")
   			   totalValue = 200;
			else if ((kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPArtificialIslands") &&
			    (mapType < 400))
   			   totalValue = 10;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPWarHutTraining")
   			   totalValue = 597;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCShipFoodCrates3")
   			   totalValue = 599;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPShipMedicineMen2Aztec")
   			   totalValue = 601;
			else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPCoyoteCombat")
   			   totalValue = 602;
   			   ...
   			   totalValue = 903;
			else if ((kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPTempleCenteotl") ||
					 (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPTempleXipeTotec"))
   			   totalValue = 901;
			...
			//else if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPCoinCratesAztec3")
   			   ...
   			   totalValue = 600;
			else if ((kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPKnightHitpoints") ||
			   ...
			   totalValue = 898;
				...
				(mapType > 400))
	   		   ...
	   		   totalValue = 607;
					 ...
					 (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, i)) == "HCXPShipVillagers4"))
   			   totalValue = 606;
...
void shipGrantedHandler(int parm=-1) // Event handler
{
   ...
   for (i=0; < totalCards)
   {
	...
	if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, bestCard)) == "HCXPCoinCratesAztec3")
	   xsArraySetInt(gCardValues, bestCard, xsArrayGetInt(gCardValues, bestCard) - 2);//599
	//jap
	if (kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, bestCard)) == "YPHCShipBerryWagon2")
	{
	   xsArraySetInt(gCardValues, bestCard, 300);
	   gCardSent = 1;
	}
	//sioux
	if ((kbGetTechName(aiHCDeckGetCardTechID(gDefaultDeck, bestCard)) == "HCShipWoodCrates3") && (cMyCiv == cCivXPSioux))
	{
	   gCardSent = 1;
	}
	...
	if ((xsArrayGetInt(gCardValues, bestCard) < 201) && (cMyCiv == cCivJapanese) && (kbGetAge() == cAge2))
	{
		return;
	}
...
minInterval 10
{
   ...
   if(kbGetCiv() == cCivOttomans && xsGetTime() < 480000)
      return;
   if(kbGetCiv() == cCivIndians && xsGetTime() < 600000)
      return;
...
minInterval 10
{
   ...
   if(cMyCiv == cCivIndians && xsGetTime() > 600000)
   {
      if (activePlan != -1)
	     aiPlanDestroy(activePlan);
   ...
   if(gFbVec == cInvalidVector)
   {
	  ...
	  else if(times == 1 && kbGetCiv() == cCivJapanese && getAllyCount2() < 1)
	  {
	     gFbVec = kbUnitGetPosition(getUnit(def));
	  }
   ...
   if(times == 8)
	{
	   ...
	   if(civIsNative() == true && xsIsRuleEnabled("randomWalk") == false)
	      xsEnableRule("randomWalk"); 
...
mininterval 30
{
   ...
   if (kbUnitCount(cMyID, cUnitTypeypWCPorcelainTower4, cUnitStateAlive) > 0)
      WCType = cUnitTypeypWCPorcelainTower4;
   if (WCType > 0)
   {
      int QueryID = -1;
...
{  
   ...
   if(kbGetCiv() == cCivOttomans && kbUnitCount(cMyID, gHouseUnit, cUnitStateAlive) < 7)
      return;
   
   if(kbGetCiv() == cCivFrench && kbGetAge() > cAge1 && kbTechGetStatus(cTechPlacerMines) == cTechStatusObtainable)//just go for FF
   {
   ...
   else if (kbGetCiv() == cCivGermans && kbGetAge() > cAge1 && kbTechGetStatus(cTechGangsaw) == cTechStatusObtainable)
   {
   ...
   else if(kbGetAge() < cAge2 || kbUnitCount(cMyID, gHouseUnit, cUnitStateAlive) < 4)
      return;
...
{  
   ...
   if(kbUnitCount(cMyID, gHouseUnit, cUnitStateAlive) < 8 && cMyCiv == cCivXPAztec)
      return;
...
//gather a bit resource for market upgrade,etc
rule initGather
inactive
group tcComplete
mininterval 5
...
//modify from towerManager for dock protection
rule towerManager2
inactive
minInterval 15
{
...
mininterval 10
{
   ...
   if((kbResourceGet(cResourceGold) > 1400) && (kbResourceGet(cResourceFood) < 1200))
   {
      aiBuyResourceOnMarket(cResourceFood);
   }
...
mininterval 15
{
   ...
   if(kbGetAge() > cAge1)
   {
	  ...
	  {
		 xsEnableRule("houseMonitor");
		 xsEnableRule("preMilitaryManager");
		 xsEnableRule("FFmonitor");
		 return;
	  }
	  else if(cMyCiv == cCivXPAztec)
	  {
   ...
   if (woodPlanID < 0)
      {  // Initialize the plan
	     ...
	     if(cMyCiv != cCivGermans)
		    ...
		    aiPlanAddUnitType(woodPlanID, cUnitTypeSettlerWagon, 1, 1, 1);
         aiPlanSetDesiredPriority(woodPlanID, 95);
         aiPlanSetActive(woodPlanID);
      }
}
...
mininterval 15
{
   ...
   //force AI to make some units directly
   if(cMyCiv == cCivOttomans)
   {
      u1 = cUnitTypeJanissary;
   }
   ...
   //prepare fb
   if(cMyCiv == cCivXPSioux || cMyCiv == cCivXPIroquois || cMyCiv == cCivXPAztec || cMyCiv == cCivJapanese)
      return;
...
{  // Broke this out separately (from navyManager) so that scenarios that start with a pre-made navy will work.
   ...
   static int q1 = 0;
   if (cvInactiveAI == true || cMyCiv == cCivGermans || cMyCiv == cCivFrench || cMyCiv == cCivOttomans || cMyCiv == cCivRussians)
   {
      xsDisableSelf();
      return;
   }
...
minInterval 15
{
   if (kbGetCiv() == cCivXPSioux || kbGetCiv() == cCivXPAztec)
   {
      xsDisableSelf();
      return;
   }
...
minInterval 30
{
   ...
   int numTroop = kbUnitCount(cMyID, cUnitTypeJanissary, cUnitStateAlive);
   int PlanID = -1;
   if(kbGetAge() < cAge3 || numTroop < 20)
      return;
   ...
   {
      PlanID = aiPlanGetIDByTypeAndVariableType(cPlanResearch, cResearchPlanTechID, cTechVeteranJanissaries);
      if (PlanID < 0)
         createSimpleResearchPlan(cTechVeteranJanissaries, getUnit(cUnitTypeBarracks),cMilitaryEscrowID, 86);
   }
